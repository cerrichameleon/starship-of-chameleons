...
One-shot message (live provider by default):

```bash
PYTHONPATH=src python3 examples/captain_chat.py "Captain, give me a bridge update"
```

Interactive mode:

```bash
PYTHONPATH=src python3 examples/captain_chat.py --interactive
```

Session status:

```bash
PYTHONPATH=src python3 examples/captain_chat.py --status
```

If you want the toy fallback brain instead of the live provider-backed one:
...
1. let Captain delegate to internal Chameleons
2. let Chief Engineer outfit Chameleons with role-appropriate brains
3. add resilience supervision and failure recovery
4. move from local Captain chat to Telegram Captain bridge
